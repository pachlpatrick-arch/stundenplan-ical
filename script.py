...
def fetch_timetable():
    ...
    try:
        ...
        if login_res.status_code != 200:
            print(f"Login fehlgeschlagen. Statuscode: {login_res.status_code}")
            return
            
        login_json = login_res.json()
        person_id = login_json.get("data", {}).get("personId")
        if not person_id:
            print("Login war erfolgreich, aber keine personId gefunden.")
            return
    ...
    for week_offset in range(0, 4):
        ...
        try:
            ...
            print(f"-> {len(periods)} Termine in dieser Woche gefunden.")
            ...
            print(f"Fehler bei Woche {target_date}: {e}")

    # 3. Schritt: Speichern der fertigen iCal-Datei
    ical_content = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
